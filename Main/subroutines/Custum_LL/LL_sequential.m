...
%% Loop trough chains/sets of observations
    ...
    parfor ii = 1:size(params,1)  
        ...
        if ~isempty(AnParam.COVdata)
            ...
            Diagonal_sigma = polys*wCurr *eye(AnParam.jj);  
        ...
        for jj = 1:nExp    
             ...
             %calculate the LL
             deltay = (y(jj,:) -YPCE(ii,:))./y(jj,:);
             wLL = ones(1,AnParam.jj)';SE = SE*eye(AnParam.jj);
             logLikeli =  logLikeli  -0.5 * logCdet - 0.5*(deltay.*wLL')*(Cinv +SE )*(deltay'.*wLL); 

        end

        %consider N_exp and length of coord
        logL(ii) = logLikeli/AnParam.N_Exp/numel(coord)/numel(y);%/N_length_obs /N_exp
