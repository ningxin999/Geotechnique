function Polybasis = legendrePolynomialBasis(Order,coord)
%Addme: create a function to generate the legendre polynomial basis
%Step one: rescale the coord to [-1,1]
%Step two: create the basis

%Input: Order: the maximum order of polynomials e.g., 6 = 0,1,2,3,4,5,6
%Output: Polybasis

% scale the coordination to [-1,1]
scaled_Coord = linspace(-1,1,numel(coord))';

% give a empty polybasis to restore all polybasis
Polybasis = [];

%loop to get the polybasis
for ii = 0: Order 
    [Polybasis_temp,~] = legendrePrealization(ii,scaled_Coord);
    Polybasis= horzcat(Polybasis,Polybasis_temp);
end
...
function [p, err]=legendrePrealization(l,m,x)
