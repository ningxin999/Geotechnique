...
% To run this file you need to have two inputs in the workspace
% 1. FE_ID = 1
% 2. outputfilename = 'E:\tl\RUN\Triaxial\matlab_v2\list.t'
...
% Construct the path for the input file and read instructions
    ...
    for k = 1:length(instruction_cell)
        ...
        if type == 1
            ...
            search_text_line1 = sprintf('    GRAPH TYPE  %d COMMENCES.', x_var);

            for i=1:length(file_content)
                if  contains(file_content(i),incrementlist)
                    flag1 = true;  % First condition has been met twice
                end

                if  contains(file_content(i),search_text_line1) && flag1 
                    flag2 = true;
                end
    ...
    %                         coord{k} = t(:,5)';  % coordinates for output1
                            ...
                            % Reset flags after the data extraction
                            flag1 = false;  % Reset flag1 so it can be triggered again
                            flag2 = false;  % Reset flag2 to wait for the next match
                end
            end
        end
    end
...
% create a new folder to save the results
    subfolder = fullfile(pwd,'output', 'mat','EoD_FE_current' ); 
